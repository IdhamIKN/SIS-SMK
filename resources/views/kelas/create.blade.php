...
<div class="kelas-wrap">
    ...
    <div class="page-strip page-strip-kelas">
        ...
        <p>Isi informasi kelas dengan lengkap dan benar</p>
    </div>
    ...
    <form id="createKelasForm" method="POST" action="{{ route('kelas.store') }}">
        @csrf
                        ...
                        @foreach (['X' => 'Sepuluh', 'XI' => 'Sebelas', 'XII' => 'Dua Belas'] as $val => $lbl)
                            <label class="tingkat-card">
                                <input type="radio" name="tingkat" value="{{ $val }}"
                                    {{ old('tingkat') === $val ? 'checked' : '' }} required>
                    ...
                    <div class="shift-group">
                        <label class="shift-card pagi">
                            <input type="radio" name="shift" value="Pagi"
                                {{ old('shift') === 'Pagi' ? 'checked' : '' }} required>
            ...
            <div class="form-card-body">
                ...
                <div class="fgroup">
                    ...
                    <select id="bk_id" name="bk_id"
                        ...
                        <option value="">— Belum ditentukan —</option>
                        @foreach ($gtks as $g)
                            <option value="{{ $g->id }}" {{ old('bk_id') == $g->id ? 'selected' : '' }}>
                                {{ $g->nama_lengkap }}{{ $g->kd_guru ? ' (' . $g->kd_guru . ')' : '' }}
                            </option>
                        @endforeach
                    </select>
...
<div class="action-bar">
    <a href="{{ route('kelas.index') }}" class="ab-btn ab-btn-back">
        <i class="fas fa-times"></i> Batal
    </a>
    <button type="submit" form="createKelasForm" class="ab-btn ab-btn-primary">
