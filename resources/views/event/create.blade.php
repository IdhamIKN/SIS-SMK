...
@push('styles')
    ...
    <style>
        ...
        .absen-toggle-card {
            position: relative;
            /* anchor untuk checkbox overlay */
            border-radius: 12px;
            overflow: hidden;
            /* biar border-radius terapply ke isi */
        }

        /* Checkbox dibesarkan jadi seluruh area kartu = area klik */
        .absen-toggle-card input[type="checkbox"] {
            ...
            cursor: pointer;
            z-index: 2;
            /* di atas label, jadi tap target sesungguhnya */
            margin: 0;
        }

        /* Label bersifat visual saja, z-index di bawah checkbox */
        .absen-toggle-label {
            position: relative;
            z-index: 1;
            display: flex;
            ...
            transition: all .18s;
            pointer-events: none;
            /* klik diteruskan ke checkbox di atasnya */
        }

        /* State: belum dicentang */
        .absen-toggle-icon {
            font-size: 1.4rem;
        }
...
@section('content')
    <div class="event-wrap" style="padding-bottom: calc(var(--footer-h) + 88px);">
        ...
        <form id="createForm" method="POST" action="{{ route('event.store') }}">
            @csrf
            ...
            <div class="card">
                ...
                <div class="c-body" style="padding:16px;">
                    ...
                    {{-- Pilihan absen: checkbox overlay penuh area kartu --}}
    ...
    <div class="c-body" style="padding:16px;">
        ...
        <div class="mode-radio-wrap">
            <div class="mode-radio">
                <input type="radio" name="mode_peserta" value="kelas"
                    {{ old('mode_peserta', 'kelas') === 'kelas' ? 'checked' : '' }}
                    onchange="toggleMode()">
                <div class="mode-box">
            ...
            <div class="mode-radio">
                <input type="radio" name="mode_peserta" value="siswa"
                    {{ old('mode_peserta') === 'siswa' ? 'checked' : '' }}
                    onchange="toggleMode()">
                <div class="mode-box">
        ...
        <label class="check-row">
            <input type="checkbox" name="berlaku_untuk_semua" id="berlakuSemua" value="1"
                {{ old('berlaku_untuk_semua', true) ? 'checked' : '' }}
        ...
        @php
            ...
            // Pre-selected (dari old() saat validasi gagal)
            $initialKelas = collect(old('kelas_id', []))->map(fn($id) => ['id' => $id, 'text' => \App\Models\Kelas::find($id)?->nama_kelas ?? $id, 'meta' => ''])->values()->toArray();
            $initialSiswa = collect(old('siswa_id', []))->map(fn($id) => ['id' => $id, 'text' => \App\Models\Siswa::find($id)?->nama_lengkap ?? $id, 'meta' => ''])->values()->toArray();
        @endphp

        <div id="kelasGroup" style="display:{{ $isSemua ? 'none' : ($mode === 'kelas' ? 'block' : 'none') }};">
        ...
        <div id="siswaGroup" style="display:{{ $isSemua ? 'none' : ($mode === 'siswa' ? 'block' : 'none') }};">
            @include('components.ajax-multi-select', [
                'selectId'    => 'siswa',
                'inputName'   => 'siswa_id[]',
